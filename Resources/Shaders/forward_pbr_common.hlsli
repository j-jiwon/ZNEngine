// Shared body of the forward PBR shader used by the offscreen camera passes (CCTV, VehicleScene's
// surround views). Everything here is independent of HOW the world matrix arrives, so the two
// entry-point files -- forward_pbr.hlsli (per-object world in cbTransform) and
// forward_pbr_instanced.hlsli (per-instance world from a StructuredBuffer) -- include this and
// supply only their own b0 layout + VS_Main.
//
